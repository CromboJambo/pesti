...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let _model_cpu = pesti_runner::LlamaRunner::builder(model_path).build()?;
    ...
    println!(
        "  Output preview: \"{}...\"",
        &result_cpu.text[..result_cpu.text.len().min(50)]
    );
    ...
    println!(
        "  CPU output: \"{}\"",
        &result_cpu.text[..result_cpu.text.len().min(60)]
    );
