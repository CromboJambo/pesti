...
fn test_one_stage_attention() {
    ...
    let output_ptr = unsafe { pesti_runner::cuda_runtime::allocate_device_memory(output_buffer_bytes).unwrap() };
    ...
    unsafe {
        ...
        pesti_runner::cuda_runtime::copy_host_to_device(k_ptr, k_h_f16.as_ptr() as *const u8, k_size)
            .unwrap();
        pesti_runner::cuda_runtime::copy_host_to_device(v_ptr, v_h_f16.as_ptr() as *const u8, v_size)
            .unwrap();
    }
