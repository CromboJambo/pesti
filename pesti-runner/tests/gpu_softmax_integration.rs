...
mod tests {
    ...
    fn test_cpu_softmax_basic() {
        ...
        let probs = softmax_kernel.forward(&logits).expect("softmax should succeed");
        ...
        assert!(probs.iter().all(|&p| p > 0.0), "All softmax values should be positive");
        
        // Verify numerical stability with large values
        let large_logits = vec![1000.0, 1001.0, 1002.0];
        let stable_probs = softmax_kernel.forward(&large_logits).expect("softmax should handle large values");
        let stable_sum: f32 = stable_probs.iter().sum();
        assert!((stable_sum - 1.0).abs() < 1e-5, "Stable softmax should sum to ~1.0, got {}", stable_sum);
