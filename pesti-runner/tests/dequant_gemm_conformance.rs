...
fn cpu_dequant_gemm_q4k(
    ...
    cpu_gemm.gemm_f32(&a_f32, &b_f32, &mut c, m, n, k, alpha, beta)
...
fn test_dequant_gemm_case<F>(
    ...
    k: usize,
    make_a_packed: F,
    cpu_ref: fn(&[u8], &[f16], Option<&[f32]>, usize, usize, usize, f32, f32) -> Result<Vec<f32>, String>,
) where
    F: FnOnce() -> (Vec<u8>, Vec<f16>),
{
    ...
    let cpu_result = cpu_ref(&a_packed, &b_f16, None, m, n, k, 1.0, 0.0)
    ...
    let a_f32 = match name {
        n if n.contains("Q4_K") => dequantize_q4_k_tile(&a_packed, 0, m * k).expect("dequant failed"),
        n if n.contains("Q4_0") => dequantize_q4_0_tile(&a_packed, 0, m * k).expect("dequant failed"),
        n if n.contains("Q8_0") => dequantize_q8_0_tile(&a_packed, 0, m * k).expect("dequant failed"),
    ...
    println!("{}: max diff = {:.6e} at idx {} (cpu={:.6}, gpu={:.6})", 
...
fn test_dequant_gemm_vs_real_model_weights() {
    ...
    println!("Testing tensor {}: {}x{}", tensor_name, in_features, out_features);
    ...
    assert!(max_diff < 1e-2, "Real model conformance failed: max diff {:.6e}", max_diff);
...
fn test_block_dequant_exact() {
    ...
    let block_data = [
        ...
        // delta = 0.5
        0x00, 0x38,
        // qs_low = 0x44444444 (q=4 for all 8)
        0x44, 0x44, 0x44, 0x44,
        // qs_high = 0x88888888 (q=8 for all 8)
        0x88, 0x88, 0x88, 0x88,
        // h[0] = 1.0
        0x00, 0x3C,
        // h[1] = 2.0
