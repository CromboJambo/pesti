...
fn test_inertia_manager_basic() {
    ...
    let result = manager.request_work(
        pesti_runner::inertia::WorkType::Gemm {
    ...
    println!("  ✓ GPU returned → drained {} pending work item(s)", pending.len());

    // Check stats
    let stats = manager.stats();
    assert_eq!(stats.total_work_logged, 2);
    assert_eq!(stats.total_work_executed, 1);
    println!("  ✓ Stats: logged={}, executed={}", 
...
fn test_gpu_dropout_scenario() {
    ...
    manager.set_gpu_available(true);
    for _ in 0..10 {
        manager.request_work(
            pesti_runner::inertia::WorkType::Gemm {
                m: 64, n: 64, k: 2048, alpha: 1.0, beta: 0.0,
            }
        );
    }
    ...
    for _ in 0..20 {
        let result = manager.request_work(
            pesti_runner::inertia::WorkType::Attention {
                query_seq_len: 1,
                num_heads: 8,
                head_dim: 64,
                cache_seq_len: 128,
    ...
    println!("  Phase 2: GPU dropout → {} works logged to queue", logged_count);

    // Phase 3: GPU recovery
    manager.set_gpu_available(true);
    let pending = manager.get_pending_for_execution();
    println!("  Phase 3: GPU recovered → {} pending items available for replay", pending.len());

    // Check stats show the full lifecycle
    let stats = manager.stats();
    assert_eq!(stats.total_work_logged, 30); // 10 + 20
    println!("  ✓ Total logged: {}, pending drained: {}", 
             stats.total_work_logged, pending.len());
