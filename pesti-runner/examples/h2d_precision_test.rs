...
fn main() {
    ...
    for (i, (&orig, &converted)) in test_values.iter().zip(old_path_result.iter()).enumerate() {
        ...
        println!(
            "  [{}] orig={:.8}, conv={:.8}, diff={:.10e}",
            i, orig, converted, diff
        );
    }
    ...
    let old_max_error: f32 = test_values
        .iter()
        .zip(old_path_result.iter())
        .map(|(a, b)| (a - b).abs())
        .fold(0.0f32, f32::max);

    let new_max_error: f32 = test_values
        .iter()
    ...
    println!(
        "Converted to f16 (old path intermediate): {:?}",
        old_softmax_f16
            .iter()
            .map(|&x| x.to_f32())
            .collect::<Vec<_>>()
    ...
    let precision_loss = attention_scores
        .iter()
    ...
    println!(
        "Total precision loss from intermediate f16 conversion: {:.8e}",
        precision_loss
    );
