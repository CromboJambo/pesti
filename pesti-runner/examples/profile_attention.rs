...
use pesti_runner::cuda_runtime::{CudaRuntime, init_cuda};
use pesti_runner::kernel::attention::{AttentionArch, AttentionConfig};
use pesti_runner::kernel::gemm::{CudaGemmKernel, GemmArch};
use pesti_runner::kernel::softmax::{SoftmaxBackend, SoftmaxKernelBuilder};
...
fn main() {
    ...
    unsafe {
        cudarc::driver::cudaMemcpyAsync(
            q_buf.ptr(),
            q_data.as_ptr() as *const _,
            q_size * 2,
            cudarc::driver::MemcpyKind::HostToDevice,
            rt.stream().as_raw(),
        );
        cudarc::driver::cudaMemcpyAsync(
            k_buf.ptr(),
    ...
    let scores_buf =
        pesti_runner::kernel::memory::CudaMemoryBackend::alloc_device(&rt, scores_size * 4)
    ...
    unsafe {
        cudarc::driver::cudaMemcpyAsync(
            scores_host.as_mut_ptr() as *mut _,
            scores_buf.ptr(),
            scores_size * 4,
            cudarc::driver::MemcpyKind::DeviceToHost,
            rt.stream().as_raw(),
        );
    }
    pesti_runner::cuda_shim::stream_synchronize(rt.stream()).unwrap();
    ...
    let softmax_scores = softmax
        .softmax(&scores_host, query_seq_len * num_heads, cache_seq_len)
        .expect("softmax failed");
    let softmax_time = t2.elapsed().as_secs_f64() * 1000.0 - transfer_time;

    // Phase 4: Transfer scores back to device
    let softmax_buf =
        pesti_runner::kernel::memory::CudaMemoryBackend::alloc_device(&rt, scores_size * 2)
            .expect("softmax buf alloc failed");
    let t3 = Instant::now();
    unsafe {
        cudarc::driver::cudaMemcpyAsync(
            softmax_buf.ptr(),
            softmax_scores.as_ptr() as *const _,
            scores_size * 2,
            cudarc::driver::MemcpyKind::HostToDevice,
            rt.stream().as_raw(),
        );
    ...
    let output_buf =
        pesti_runner::kernel::memory::CudaMemoryBackend::alloc_device(&rt, output_size * 4)
    ...
    println!(
        "Phase 1 - Q @ K^T GEMM:     {:.2} ms ({:.1}%)",
        gemm_time,
        gemm_time / total_time * 100.0
    );
    println!(
        "Phase 2 - Transfer to host:  {:.2} ms ({:.1}%)",
        transfer_time,
        transfer_time / total_time * 100.0
    );
    println!(
        "Phase 3 - Softmax (CPU):     {:.2} ms ({:.1}%)",
        softmax_time,
        softmax_time / total_time * 100.0
    );
    println!(
        "Phase 4 - Transfer to dev:   {:.2} ms ({:.1}%)",
        transfer_back_time,
        transfer_back_time / total_time * 100.0
    );
    println!(
        "Phase 5 - S @ V GEMM:        {:.2} ms ({:.1}%)",
        gemm2_time,
        gemm2_time / total_time * 100.0
    );
    println!("--------------------------------------------------");
