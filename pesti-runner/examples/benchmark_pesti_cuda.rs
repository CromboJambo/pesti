...
fn main() {
    ...
    let model = match pesti_runner::transformer::model::LlamaModel::load_gguf(Path::new(model_path)) {
        Ok(m) => m,
        Err(e) => {
    ...
    // Tokenize prompt
    let tokenizer = match pesti_runner::transformer::tokenizer::load_tokenizer_from_gguf(Path::new(model_path)) {
        Ok(t) => t,
        Err(e) => {
