...
fn main() {
    ...
    println!(
        "Min time: {:.3}ms",
        (total_time / iterations).as_secs_f64() * 1000.0
    ); // Simplified
    println!(
    ...
    println!(
        "  - Use: cargo run --package pesti-runner --example compare_llama_vs_pesti --features cuda,llama-cpp"
    );
    println!("  - Note: Requires llama.cpp context initialization with model weights");
    println!("\nFor mistralrs comparison:");
    println!(
        "  - Use: cargo run --package pesti-runner --example compare_llama_vs_pesti --features cuda,mistralrs"
    );
    println!("  - Note: Requires GPU kernels and model loading overhead");
    println!(
        "\nPure CPU benchmark (this run): {:.3}ms",
