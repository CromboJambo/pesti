...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  seq_q={}, seq_k={}, heads={}, dim={}", seq_q, seq_k, num_heads, head_dim);
    println!();
    ...
    unsafe {
        kernel.launch(
            scale, q_ptr as u64, k_ptr as u64, v_ptr as u64, s_ptr as u64,
    ...
    println!("  Throughput: {:.1} runs/sec", num_runs as f64 / avg_kernel_time.as_secs_f64());
    ...
    println!("H2D + D2H overhead: {} µs", (h2d_time + d2h_time).as_micros());
    println!("Compute fraction: {:.1}%", 100.0 * avg_kernel_time.as_micros() as f64 / 
