...
fn main() {
    ...
    let result = ql.forward(&x, 1);
    println!(
        "Q4_K large output: len={}, all_zero={}",
        result.len(),
        result.iter().all(|&v| v == 0.0)
    );
