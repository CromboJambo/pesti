...
fn run_benchmark(config: &BenchmarkConfig) -> (f64, f64) {
    ...
    // Warm-up
    let _ = reference_with_ndarray(&q_h, &k_h, &v_h, config.seq_q, config.seq_k, config.num_heads, config.head_dim, rope_base, scale);
    ...
    for _ in 0..iterations {
        ...
        let _result = reference_with_ndarray(&q_h, &k_h, &v_h, config.seq_q, config.seq_k, config.num_heads, config.head_dim, rope_base, scale);
    ...
    let total_ops = config.seq_q as f64 * config.seq_k as f64 * config.num_heads as f64 * config.head_dim as f64 * 3.0; // Multiply by 3 for all operations
...
fn main() {
    ...
    let configs = vec![
        // Small scale
        BenchmarkConfig { seq_q: 32, seq_k: 32, num_heads: 4, head_dim: 64 },
        BenchmarkConfig { seq_q: 64, seq_k: 64, num_heads: 4, head_dim: 64 },
        
        // Medium scale (current baseline)
        BenchmarkConfig { seq_q: 128, seq_k: 128, num_heads: 4, head_dim: 64 },
        
        // Large scale
        BenchmarkConfig { seq_q: 256, seq_k: 256, num_heads: 8, head_dim: 128 },
    ...
    for config in &configs {
        ...
        let total_ops = config.seq_q as f64 * config.seq_k as f64 * config.num_heads as f64 * config.head_dim as f64;
        let ops_per_sec = total_ops / time_ms / 1e-3;

        println!("{:<35} {:>12.3} {:>15.2} {:>12.2e}", 
                 format!("seq={}x{}, heads={}, dim={}", config.seq_q, config.seq_k, config.num_heads, config.head_dim),
                 time_ms, throughput_mops, ops_per_sec);
    }
    ...
    println!("Largest config ({}x{}, {} heads, {} dim):", 
    ...
    let avg_time_ms = (max_config.seq_q as f64 * max_config.seq_k as f64 * max_config.num_heads as f64 * max_config.head_dim as f64 * 3.0 / 1e6) / last_throughput_mops;
