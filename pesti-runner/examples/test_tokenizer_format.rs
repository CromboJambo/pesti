...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for id in 0..token_count.min(100) { // Only first 100 for testing
    ...
    println!("\nFirst 5 tokens: {:?}", &added_tokens[..5.min(added_tokens.len())]);
    
    // Try building tokenizer with CORRECT format (map instead of array)
    let vocab_map: std::collections::HashMap<String, u32> = added_tokens.iter()
    ...
    println!("\nConfig JSON (truncated): {}", 
        serde_json::to_string(&config_json).unwrap_or_default().chars().take(200).collect::<String>());
            ...
            match tokenizer.encode(prompt, true) {
                Ok(encoding) => {
                    ...
                    if !encoding.get_ids().is_empty() {
                        println!("First 10 IDs: {:?}", &encoding.get_ids()[..10.min(encoding.get_ids().len())]);
                    }
                }
                Err(e) => println!("\n❌ Encode error: {}", e),
            }
