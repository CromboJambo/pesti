...
use pesti_runner::{LlamaRunnerBuilder, KvCacheType};
...
fn bench_kv_cache(model_path: &str, kv_type: KvCacheType, label: &str) -> f64 {
    ...
    let warmup_config = SamplingConfig { max_tokens: 5, ..Default::default() };
    ...
    // Timed run
    let config = SamplingConfig { max_tokens: 10, ..Default::default() };
    ...
    println!("Decode speed: {:.2} tok/s ({:.0}ms/token)", tok_per_sec, 1000.0 / tok_per_sec.max(0.001));
    println!();
