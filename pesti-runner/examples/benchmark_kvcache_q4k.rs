...
fn main() {
    ...
    println!("  FP16 KV cache: {} bytes ({:.2} KB)", fp16_bytes_per_layer, fp16_bytes_per_layer as f64 / 1024.0);
    println!("  Q4_K KV cache: {} bytes ({:.2} KB)", q4k_bytes_per_layer, q4k_bytes_per_layer as f64 / 1024.0);
    println!();
    ...
    println!("  FP16: {} bytes ({:.2} MB)", total_fp16, total_fp16 as f64 / (1024.0 * 1024.0));
    println!("  Q4_K: {} bytes ({:.2} MB)", total_q4k, total_q4k as f64 / (1024.0 * 1024.0));
    println!();

    let savings = ((total_fp16 as f64 - total_q4k as f64) / total_fp16 as f64) * 100.0;
    println!("Memory savings: {:.1}% ({:.2} MB saved)", savings, (total_fp16 - total_q4k) as f64 / (1024.0 * 1024.0));
    println!();
    ...
    println!("Memory bandwidth during decode (seq={}, 1 token generated):", seq_len);
    println!("  FP16: {} bytes ({:.2} KB)", kv_read_bytes_per_token_fp16, kv_read_bytes_per_token_fp16 as f64 / 1024.0);
    println!("  Q4_K: {} bytes ({:.2} KB)", kv_read_bytes_per_token_q4k, kv_read_bytes_per_token_q4k as f64 / 1024.0);

    let bw_savings = ((kv_read_bytes_per_token_fp16 as f64 - kv_read_bytes_per_token_q4k as f64) / kv_read_bytes_per_token_fp16 as f64) * 100.0;
    println!("  Bandwidth savings: {:.1}%", bw_savings);
    println!();

    // Estimated throughput improvement (memory-bound operation)
    let estimated_speedup = kv_read_bytes_per_token_fp16 as f64 / kv_read_bytes_per_token_q4k as f64;
    println!("Estimated attention kernel speedup (if memory-bound): {:.2}x", estimated_speedup);
