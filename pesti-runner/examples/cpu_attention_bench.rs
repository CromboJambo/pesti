...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (num_heads, head_dim, seq_len, label) in configs {
        ...
        // Create Kvcache
        let mut kvc = pesti_runner::kernel::Kvcache::new(num_heads, num_heads, head_dim, seq_len, false);
        let head_stride = num_heads * head_dim;
        for pos in 0..seq_len {
            kvc.write_kv_at(pos, &k_host[pos * head_stride..(pos + 1) * head_stride], 
        ...
        println!("  Time: {:.2}ms/iter, {:.1} tok/s", 
                 elapsed.as_secs_f64() * 1000.0, tok_s);
