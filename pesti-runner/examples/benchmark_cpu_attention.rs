...
fn main() {
    ...
    println!(
        "  Time: {:.2}ms ({} iterations)",
        naive_time.as_millis(),
        iterations
    );
    ...
    let optimized_output = reference_raw_scores_optimized(
        &q_h, &k_h, &v_h, seq_q, seq_k, num_heads, head_dim, rope_base, scale,
    );
    ...
    println!(
        "\n✅ Numerical check: Max difference between implementations: {:.6e}",
        max_diff
    );
