...
async fn main() -> pesti_runner::error::Result<()> {
    ...
    println!(
        "Generating {} tokens with greedy sampling...",
        config.max_tokens
    );
    ...
    if let Some(runner) = runtime.model_info().await {
        ...
        if !result.tokens.is_empty() {
            ...
            println!(
                "\nFirst 10 new token IDs: {:?}",
                &new_tokens[..std::cmp::min(10, new_tokens.len())]
            );

            // Try to get text representation
            let text_len = std::cmp::min(200, result.text.len());
            println!(
                "Generated text (first {} chars): \"{}...\"",
                text_len,
    ...
    if result.generated_tokens == result2.generated_tokens {
        ...
        if new_tokens1 == new_tokens2 {
            ...
            println!(
                "   Run 1 first 5: {:?}",
                &new_tokens1[..std::cmp::min(5, new_tokens1.len())]
            );
            println!(
                "   Run 2 first 5: {:?}",
                &new_tokens2[..std::cmp::min(5, new_tokens2.len())]
            );
        }
    } else {
