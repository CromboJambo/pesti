...
use pesti_runner::kernel::{
    flash_attention_v2::FlashAttentionKernel,
    batched_parallel_attention::BatchedParallelAttentionKernel,
    fused_linear_attention::FusedLinearAttentionKernel,
};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Savings: {:.1}%\n", ((kv_cache_memory_fp32 - kv_cache_memory_fp16) as f32 / kv_cache_memory_fp32 as f32) * 100.0);
    ...
    println!("Batched (4 sequences) throughput: {:.2} tok/s", throughput_batched);
    ...
    println!("Memory savings: {:.1}% (for seq_len=2048)", flash_kernel.memory_savings_percentage());
