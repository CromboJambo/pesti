...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let gpu_available = match Device::cuda_if_available(0) {
        Ok(gpu_device) => {
            ...
            println!("   Attention available: {}", gpu_engine.attention_available());
    ...
    if model_exists {
        ...
        println!("   ✅ Model already exists ({:.1} MB)", metadata.len() as f64 / 1024.0 / 1024.0);
    } else {
        ...
        println!("   💡 To download: huggingface-cli download Qwen/Qwen2.5-0.5B-Instruct-GGUF qwen2.5-0.5b-instruct-q4_k_m.gguf --local-dir /tmp/");
    }
    ...
    if model_exists && gpu_available {
        ...
        println!("   GPU engine initialization: {:.3}s", gpu_time.as_secs_f64());
    }
    ...
    if model_exists {
        ...
        println!("   1. Download model: huggingface-cli download Qwen/Qwen2.5-0.5B-Instruct-GGUF qwen2.5-0.5b-instruct-q4_k_m.gguf --local-dir /tmp/");
