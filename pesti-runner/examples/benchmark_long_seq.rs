...
use pesti_runner::{LlamaModel, TokenizerBackend, load_gguf_weights};
...
fn main() {
    ...
    for &seq_len in &seq_lengths {
        ...
        let _result = model
            .generate(&prompt_tokens, 1, &sampling, &mut rng, &[151663])
            .expect("Generation failed");
        let prefill_time = t_prefill.elapsed().as_secs_f64();

        println!(
            "Prefill time: {:.3}s ({:.0} tok/s effective)",
            prefill_time,
            seq_len as f64 / prefill_time
        );
        println!();
    }
