            ...
            for (name, raw_data) in &weights.raw_tensors {
                if name.contains("attention.wq.weight") || 
                   ...
                   name.contains("attention.wv.weight") {
                    ...
                    println!("Expected row_bytes: {} (for {} elements)", row_bytes, in_features);
                    println!("Expected total bytes: {} ({} rows × {} bytes/row)", 
                             out_features * row_bytes, out_features, row_bytes);
                    ...
                    if raw_data.len() >= row_bytes {
                        ...
                        println!("⚠ Not enough data for first row (got {}, need {})", 
                                 raw_data.len(), row_bytes);
                    }
