...
fn test_cpu_vs_gpu_embedding_consistency() {
    ...
    // Load model (uses CPU path for now, but GPU dispatch available)
...
fn test_cpu_vs_gpu_attention_output() {
    ...
    #[cfg(feature = "cuda")]
    {
        ...
        println!("✓ GPU dispatch enabled");
    }

    // Create test input (single token)
    let hidden_size = model.config.embed_dim;
    let input: Vec<f32> = (0..hidden_size).map(|i| (i as f32) * 0.01).collect();

    println!("Input shape: {} dimensions", input.len());

    // Run forward pass (CPU or GPU depending on dispatch)
    let start_pos = 0;
    let logits = model.forward_with_dispatch(&input, start_pos).unwrap();
    ...
    assert_eq!(logits.len() as u32, model.vocab_size, "Logit dimension mismatch");
...
fn test_cpu_vs_gpu_numerical_stability() {
    ...
    for scale in test_scales {
        ...
        for i in 0..3 {
            let logits = model.forward_with_dispatch(&input, 0).unwrap();
            outputs.push(logits);
        ...
        for output in &outputs {
            assert_eq!(output[0], outputs[0][0], "Non-deterministic output at scale {}", scale);
        }
    }
