...
- **Token-level precedent**: [Unsloth MTP guide](https://unsloth.ai/docs/models/qwen3.8-next#mtp-guide) — Qwen3.8-Flash-Next ships Multi-Token Prediction: draft several tokens, verify in parallel, keep only what verifies (1.3–1.7× faster, no accuracy loss). The fast/wild polarity at the token level: bounded verification is what keeps reach affordable.
- **Throughput budget**: [Unsloth NVFP4 guide](https://unsloth.ai/docs/basics/nvfp4) — W4A4 FP4 quantization on Blackwell (~2.5× faster). Cheaper wild steps buy more room for the slow friend to pace them.

### Test model (for G1/G5 probes)
- **MoE + MTP, fits one 16 GB card**: `unsloth/Qwen3.6-35B-A3B-MTP-GGUF` — MoE (expert-hash signal exists), ships MTP (native draft/verify probe = free G1). Quant ladder: UD-IQ2_M ≈ 11.3 GB (fits with headroom), UD-Q3_K_M ≈ 16.3 GB (borderline single-card), MXFP4_MOE ≈ 21 GB (needs both cards split or won't fit). Same model is both the high-quant bootstrap and the low-quant student — no cross-architecture KV carry needed.
- **Dense + MTP, fits one 16 GB card**: `Qwen/Qwen3-Coder-30B-A3B-Instruct` GGUF quants (unsloth) — dense, no router; discrete signature from attention-head / hidden-state-norm fingerprints instead of expert hashes.
- The HauhauCS/DavidAU Qwen3.8-27B fine-tunes are **dense** (confirmed: `model_type: qwen3_5`, no expert keys) — usable for the quant-ladder compaction test but *not* for the expert-hash checksum.

### The idea in one line
Keep a cheap always-on **stable reference on CPU** and use it to (a) preserve momentum when the GPU drops, (b) bound/edit the fast path's output, (c) **scope MoE/compute routing**, and (d) trigger **drift-gated compaction** — re-anchor both paths toward the stable summary when they diverge. Compaction is **blast-radius containment**: the slow friend does not stop the fast friend from messing up the first time — it keeps feedback latency short enough that a mistake stays a small local correction, instead of repeating (wrong state feeding back) or getting buried in patches (trajectory too far committed to unwind). Governing rule: **bounded positive gates on anything that accumulates state; hard sparsity only for compute.**
...
### Completed Nudges
- [x] **G1 — Prove the drift signal.** ✅ PASS. Divergence probe shows smooth, length-correlated growth on real model (Qwen2.5-0.5B-Instruct-Q4_K_M). Cosine divergence: 0.047 → 0.056 → 0.063 at seq_len 256/512/1024. Slow-friend ops cost ~4.6µs/step (negligible vs decode step). Commit `523be1e`.
...
- [x] **G6 — Prove compaction works.** ✅ COMPLETE. Compaction trigger with decay and re-anchoring implemented in `compaction.rs`. Re-anchor threshold tuned to avoid over-triggering; validates that drift signal can be bounded by periodic re-anchoring toward stable reference. Commit `01f957e`.
- [x] **G7 — End-to-end integration.** ✅ COMPLETE. SlowFriendState wired through TransformerLayer.forward_with_dispatch for full decode-loop integration. Summary state accessible via `summary()`/`summary_mut()` accessors. E2E test path verified; slow-friend ops run alongside normal inference without interference. Commit `01f957e`.
