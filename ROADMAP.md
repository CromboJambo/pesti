...
- [ ] **G2 — Prove scoping reduces drift.** In progress: expert-relevance prior from slow-friend state, Jaccard similarity comparison of free vs scoped activation patterns at high context.
- [ ] **G3 — Prove it's cheap enough to pace.** Per-token state maintenance (GDN update + checksum) adds < budget (target <5% step time) on CPU; divergence checks amortized every N tokens. The binding constraint is **feedback latency < error-propagation window** — the patch window must stay small, not just that the average cost is low. If too slow, shrink summary dim or use n-gram-only scoping.
